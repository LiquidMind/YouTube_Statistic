...
const { KEY32, KEY, KEY33 } = process.env;
const URL = "https://www.googleapis.com/youtube/v3/videos";

let lastId = null;
let currentKeyIndex = 0;
const apiKeys = [KEY, KEY32, KEY33]; // List of available API keys
...
setInterval(() => {
  // Першим кроком є вибір відео, де viewes = 0, okLike = 0, lengthVideo = 0 і addAllInfo = 0
  let mysqlQuery =
    "SELECT id FROM videos_all WHERE viewes = 0 AND okLike = 0 AND lengthVideo = 0 AND addAllInfo = 0 LIMIT 1";
  ...
  db.query(mysqlQuery, async function (err, results) {
    ...
    if (results.length > 0) {
      ...
      // Якщо немає нових відео, то переходимо до старих, де addAllInfo = 0
      mysqlQuery = "SELECT id FROM videos_all WHERE addAllInfo = 0 LIMIT 1";
