...
async function updatePlaylist(topX, key, orderQuery, title, description) {
  ...
  const [playlists] = await connection.execute(
    "SELECT playlist_id, title, description FROM playlists WHERE user_id = 0 AND key_playlist = ?",
    [key]
  );
  ...
  // await connection.execute(
  //   "UPDATE playlists_content_updates SET deleted_at = 'del' WHERE playlist_id = ?",
  //   [playlistId]
  // );
  ...
  await connection.execute(
    "UPDATE playlists SET title = ?, description = ?, updated_at = CURRENT_TIMESTAMP WHERE playlist_id = ?",
  ...
  await connection.execute(
    ...
    [playlistId, oldTitle, title, oldDescription, description]
  );
...
async function main() {
  ...
  await updatePlaylist(
    ...
    `ORDER BY views_per_second DESC`,
