          ...
          if (response.data.result.length === 0) {
            ...
            setVideos((prevVideos) => {
              return page === 1
                ...
                : [...prevVideos, ...response.data.result];
            });
          }
        } else {
          if (
            !response.data.result.playlist ||
            response.data.result.playlist.length === 0
          ) {
            setHasMore(false);
          } else {
            setVideos((prevVideos) => {
              return page === 1
                ? [...response.data.result.playlist]
                : [...prevVideos, ...response.data.result.playlist];
            });
          }
          if (response.data.result.deletedVideos) {
            setDeletedVideos((prevDeletedVideos) => {
              return page === 1
                ? [...response.data.result.deletedVideos]
                : [...prevDeletedVideos, ...response.data.result.deletedVideos];
            });
          }
        }
        setLoading(false);
      ...
      <ul>
        {videos.map((video, index) => (
          <VideoItem
            key={index}
            video={video}
            deletedVideos={deletedVideos} // передача deletedVideos в компонент VideoList
...
//   setHasMore(false);
// }
// if (response.data.result.playlist) {
//   setVideos((prevVideos) => {
//     if (page === 1) {
//       return [...response.data.result.playlist];
//     } else {
//       return [...prevVideos, ...response.data.result.playlist];
//     }
//   });
// }
